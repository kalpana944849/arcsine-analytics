...
import { dataGeneralSchema, dataGeneralSchemaWithoutOCCDS } from "../../../../../utils/validationSchema/validation-schema";
...
const Data = (props) => {
  ...
  const [showMoveToDifferentFolderModal, setShowMoveToDifferentFolderModal] =
    useState(false);
  ...
  const getEndpoint = async (sapID, sapVersionID, sapDataDatasetID) => {
    const response = await getSapDataEndpoints(
      sapID,
      sapVersionID,
      sapDataDatasetID
    );
  ...
  const getDataSet = async (id, versionId, context = false) => {
    const response = await getSapDataSet(id, versionId);
    if (response.status == 200) {
      if (context) {
        setSelectedRowDataContext(response.data.data);
        return;
      }
      ...
      handleChangeCodingType(
        null,
        response.data.data.sapDataDatasetCodingDictionaryId
      );
    } else {
      setDataSet({
        sapDataDatasetNameShort: "",
  ...
  let dataRowId = localStorage.getItem("dataRowId")
    ? localStorage.getItem("dataRowId")
    : "";
  ...
  const [selectedRowDataCotext, setSelectedRowDataContext] = React.useState({
    parentId: null,
  });
  ...
  const formik = useFormik({
    initialValues: {
      ...
      dataSetType: dataSet?.sapDataDatasetTypeId
        ? (dataSet?.sapDataDatasetTypeId).toString()
        : "",
      codingType: dataSet?.sapDataDatasetCodingDictionaryId,
      codingVersion: dataSet?.sapDataDatasetCodingDictionaryVersionId
    },
    enableReinitialize: true,
    validationSchema: isOCCDSSelected ? dataGeneralSchema : dataGeneralSchemaWithoutOCCDS,
    ...
    onSubmit: async (values, { resetForm }) => {
      const reqBody = {
        ...
        sapDataDatasetCodingDictionaryId: isOCCDSSelected ? values.codingType : 0,
		sapDataDatasetCodingDictionaryVersionId: isOCCDSSelected ? values.codingVersion : 0
      };
  ...
  const openContextMenu = (dataItem) => {
    ...
    // setReload(!reload)
    const id = getIdFromString(dataItem?.sapDataId);
    getDataSet(id, dataItem?.sapVersionId, true);
  ...
  const handleDeleteFolder = async () => {
    ...
    const response = await deleteSapData(selectedRowDataCotext.sapDataDatasetGuid, "test");
    if (response.status == 200) {
      getData();
	  setShowAlert(true);
    }
  ...
  useEffect(() => {
      ...
      if (status === 200) {
        ...
        data.data.forEach((item) => {
          cTypes.push({
            ...
            libraryCodingDictionaryNameShort:
              item.libraryCodingDictionaryNameShort,
          });
        });
    ...
    getCodingTypes();
  	...
  	useEffect(() => {
		const checkAndSetCodingVersion = async () => {
			if (codingTypes.length > 0) {
				await handleChangeCodingType(null, dataSet.sapDataDatasetCodingDictionaryId);
				formik.setFieldValue('codingVersion', dataSet.sapDataDatasetCodingDictionaryVersionId);
			}
		};
		checkAndSetCodingVersion();
	}, [dataSet]);
		...
		if (libraryCodingDictionary.length > 0) {
			...
			libraryCodingDictionary.forEach((item) => {
				if (codingType !== null || codingType === undefined && item.libraryCodingDictionaryId == codingType) {
					...
					if (item.libraryCodingDictionaryId == codingType) {
  ...
  return (
    <>
        ...
        {showSelected ? (
          <div className={`mt-4 tab_custom ${showSelected ? "" : "d-none"}`}>
            ...
            >
                          ...
                          >
                            <div className="col">
                              <label class="form-label">
                                Coding Type<sup className="text-danger">*</sup>
                              </label>
                              <select
                                class={`form-select form-control ${
                                  formik.touched.codingType &&
                                  formik.errors.codingType
                                    ? "is-invalid"
                                    : ""
                                }`}
                                name="codingType"
                              ...
                              >
                                ...
                                {codingTypes.length > 0 &&
                                  codingTypes.map((codingType, index) => {
                                    return (
                                      <option
                                        key={index}
                                        value={
                                          codingType.libraryCodingDictionaryId
                                        }
                                      >
                                        {
                                          codingType.libraryCodingDictionaryNameShort
                                        }
                                      </option>
                                    );
                                  })}
                            ...
                            <div className="col">
                              ...
                              <select
                                class={`form-select form-control ${
                                  formik.touched.codingVersion &&
                                  formik.errors.codingVersion
                                    ? "is-invalid"
                                    : ""
                                }`}
                                name="codingVersion"
                              ...
                              >
                                ...
                                {codingVersions.length > 0 &&
                                  codingVersions.map((codingVersion, index) => {
                                    return (
                                      <option
                                        key={index}
                                        value={
                                          codingVersion.libraryCodingDictionaryVersionId
                                        }
                                      >
                                        {
                                          codingVersion.libraryCodingDictionaryVersionNumber
                                        }
                                      </option>
                                    );
                                  })}
              ...
              <TabStripTab
                contentClassName="variable relationship"
                ...
                disabled={
                  ...
                  terminology.find(
                    (x) =>
                      x.libraryControlledTerminologyId ==
                      formik.values.dataSetType
                  )?.libraryControlledTerminologyNameShort == "ADSL" ||
                  terminology.find(
                    (x) =>
                      x.libraryControlledTerminologyId ==
                      formik.values.dataSetType
                  )?.libraryControlledTerminologyNameShort == "OCCDS"
                }
              >
                <div>
      ...
      {showMoveToDifferentFolderModal && (
        <MoveToDifferentFolder closeModal={setShowMoveToDifferentFolderModal} />
      )}
    </>
  );
};
